...
function calcularCasoActual() {
  ...
  const resultado = calcularNewtonInterpolacion(datos.dias, datos.precios, datos.x);
  ultimoResultado = resultado;

  mostrarResultado(resultado, datos);
  mostrarTablaDiferencias(resultado);
  dibujarGrafica3D(datos, resultado);
}

function calcularNewtonInterpolacion(xs, ys, x) {
  ...
  for (let j = 1; j < n; j++) {
    for (let i = 0; i < n - j; i++) {
      const denominador = xs[i + j] - xs[i];

      if (denominador === 0) {
        return {
          precio: NaN,
          estado: false,
          mensaje: "No se pueden repetir dias, porque el denominador se vuelve cero.",
          tabla: [],
          coeficientes: []
        };
      }
  ...
  return {
    precio: resultado,
    estado: true,
    mensaje: "El calculo por Newton Interpolacion se realizo correctamente.",
    tabla,
    coeficientes
  };
}
...
function mostrarResultado(resultado, datos) {
  ...
  analisis.textContent =
    ...
    " Bs. Este valor se obtiene usando la tabla de diferencias divididas.";
}

function mostrarTablaDiferencias(resultado) {
  const head = document.getElementById("tablaHead");
  const body = document.getElementById("tablaDiferencias");
  ...
  const n = resultado.tabla.length;
  ...
  for (let j = 0; j < n; j++) {
    encabezado += `<th>Orden ${j}</th>`;
  }

  encabezado += `</tr>`;
  head.innerHTML = encabezado;
  ...
  for (let i = 0; i < n; i++) {
    let fila = `
      <tr>
        <td>${i}</td>
        <td>${datos.dias[i].toFixed(4)}</td>
    `;

    for (let j = 0; j < n; j++) {
      const valor = resultado.tabla[i][j];

      if (valor === null) {
        fila += `<td>-</td>`;
      } else {
        fila += `<td>${valor.toFixed(6)}</td>`;
      }
    }
...
function compararCasos() {
  ...
  ["ideal", "moderado", "critico"].forEach((nombre) => {
    ...
    const resultado = calcularNewtonInterpolacion(caso.dias, caso.precios, caso.diaEstimado);
...
function actualizarInterpretacionVariables(nombreCaso) {
  ...
  const datos = {
    ideal: {
      texto:
        "En el estado ideal se analiza un mercado estable. Los precios aumentan lentamente y la tabla de diferencias divididas muestra cambios suaves.",
      conclusion:
        "En este caso, Newton permite estimar el precio faltante y muestra de forma ordenada como se construyen los coeficientes del polinomio."
    },
    moderado: {
      texto:
        "En la crisis moderada existe mayor presion sobre los precios. Las diferencias divididas reflejan una subida mas visible.",
      conclusion:
        "En este caso, Newton muestra que el precio estimado crece mas rapido que en el estado ideal."
    },
    critico: {
      texto:
        "En la crisis critica existe escasez y aumento acelerado. Los precios crecen rapidamente durante el mes.",
      conclusion:
        "En este caso, Newton estima un precio mas alto y evidencia una perdida fuerte del poder adquisitivo."
    }
  };
...
function dibujarGrafica3D(datos, resultado) {
  const canvas = document.getElementById("newtonInterpolationGraph");
...
function dibujarCurva(ctx, datos, punto3D) {
  ...
  for (let x = minX; x <= maxX; x += 0.2) {
    const r = calcularNewtonInterpolacion(datos.dias, datos.precios, x);
    const y = r.precio;
...
function dibujarLeyenda(ctx) {
  ...
  ctx.font = "bold 14px Segoe UI";
  ctx.fillText("Newton Interpolacion - Grafica 3D aproximada", 20, 30);
  ...
  ctx.fillStyle = "#374151";
  ctx.fillText("Curva interpolada de Newton", 42, 82);
