...
// MÓDULO 2: MÉTODO DE SOBRERRELAJACIÓN (SOR)
...
function resolverSOR(A, b, omega) {
  ...
  for (let k = 1; k <= maxIter; k++) {
    ...
    // Calcular el valor de Gauss-Seidel para x1
    let x1_gs = (b[0] - A[0][1] * x[1] - A[0][2] * x[2]) / A[0][0];
    // Aplicar relajación
    x[0] = (1 - omega) * x[0] + omega * x1_gs;

    // Calcular el valor de Gauss-Seidel para x2 (usa el nuevo x[0])
    let x2_gs = (b[1] - A[1][0] * x[0] - A[1][2] * x[2]) / A[1][1];
    // Relajación
    x[1] = (1 - omega) * x[1] + omega * x2_gs;

    // Calcular el valor de Gauss-Seidel para x3 (usa los nuevos x[0] y x[1])
    let x3_gs = (b[2] - A[2][0] * x[0] - A[2][1] * x[1]) / A[2][2];
    // Relajación
    x[2] = (1 - omega) * x[2] + omega * x3_gs;
  ...
  if (resultado.convergio) {
    info.innerHTML = `
      <p class="estado-convergio">
        ✔ El método SOR convergió.<br><br>
        Iteraciones utilizadas: <strong>${resultado.iteraciones}</strong>
      </p>
    `;
  } else {
    info.innerHTML = `
      <p class="estado-fallo">✖ El método SOR NO convergió.</p>
...
function generarAnalisis(resultado, omega) {
  ...
  // Base del análisis para cada escenario
  if (escenario === "ideal") {
    texto.innerHTML = `
      ...
      <p>
        Por esta razón SOR converge rápidamente, especialmente con un ω adecuado.
      </p>
    ...
    texto.innerHTML = `
      ...
      <p>
        El método SOR requiere más iteraciones
        para alcanzar la convergencia, pero aún puede ser eficiente con un ω bien elegido.
      </p>
      <h3>¿Qué pasa si aumenta la demanda?</h3>
      <p>
    ...
    texto.innerHTML = `
      ...
      <h3>¿Por qué SOR presenta dificultades?</h3>
      <p>
        SOR funciona mejor cuando existe dominancia diagonal.
      </p>
      <p>
    ...
    if (!resultado.convergio) {
      texto.innerHTML += `
        ...
        <p style="color:red;font-weight:bold;">
          El método SOR no logró converger dentro
    ...
    // Párrafo adicional específico para el escenario crítico
    texto.innerHTML += `
      <h3>Comportamiento de SOR bajo crisis crítica</h3>
      <p>
        Aunque la matriz se encuentra mal condicionada,
        el parámetro ω puede ayudar a acelerar
        la convergencia.
      </p>
      <p>
        Sin embargo, si ω es demasiado grande,
        el método puede divergir completamente.
      </p>
    `;
  }

  // Sección común sobre la influencia de ω (se agrega al final en todos los casos)
  texto.innerHTML += `
    <h3>Influencia del parámetro ω</h3>
    <p>
      El método SOR incorpora un parámetro de
      relajación denominado omega (ω).
    </p>
    <p>
      Cuando ω se aproxima a un valor óptimo,
      la convergencia puede ser significativamente
      más rápida que en Gauss‑Seidel.
    </p>
    <p>
      En esta simulación se usó ω = <strong>${omega}</strong>.
      El método requirió <strong>${resultado.iteraciones}</strong> iteraciones
      y ${resultado.convergio ? "sí" : "no"} alcanzó la precisión deseada.
    </p>
  `;
}
...
function ejecutarSimulacion() {
  ...
  // Leer ω desde el campo de entrada
  let omega = parseFloat(document.getElementById("omega").value);
  // Prevenir valores extremos o no numéricos
  if (isNaN(omega)) omega = 1.0;

  let resultado = resolverSOR(A, b, omega);

  mostrarResultados(resultado);
  mostrarIteraciones(resultado);
  generarAnalisis(resultado, omega);
