...
function resolverJacobi(A, b) {
  ...
  for (let k = 1; k <= maxIter; k++) {
    xNuevo[0] =
      (b[0] - A[0][1] * xAnterior[1] - A[0][2] * xAnterior[2]) / A[0][0];
    xNuevo[1] =
      (b[1] - A[1][0] * xAnterior[0] - A[1][2] * xAnterior[2]) / A[1][1];
    xNuevo[2] =
      (b[2] - A[2][0] * xAnterior[0] - A[2][1] * xAnterior[1]) / A[2][2];

    let error = Math.max(
      Math.abs(xNuevo[0] - xAnterior[0]),
      Math.abs(xNuevo[1] - xAnterior[1]),
      Math.abs(xNuevo[2] - xAnterior[2]),
    );
...
function generarAnalisis(resultado) {
  ...
  if (escenario === "ideal") {
    texto.innerHTML = `
      ...
      <p>
        Por esta razón Jacobi converge rápidamente.
      </p>
    ...
    texto.innerHTML = `
      ...
      <p>
        El método de Jacobi requiere más iteraciones
    ...
    texto.innerHTML = `
      ...
      <h3>¿Por qué Jacobi presenta dificultades?</h3>
      <p>
        Jacobi funciona mejor cuando existe dominancia diagonal.
      </p>
      <p>
    ...
    if (!resultado.convergio) {
      texto.innerHTML += `
        ...
        <p style="color:red;font-weight:bold;">
          El método de Jacobi no logró converger dentro
...
function ejecutarSimulacion() {
  ...
  let resultado = resolverJacobi(A, b);
