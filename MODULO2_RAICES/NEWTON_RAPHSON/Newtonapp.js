const casos = {
  ideal: {
    ...
    derivadaTexto:
      "f'(x) = 35 + 0.8x + 4ln(x + 1) + 1.5sen(0.3x)",
    ...
    df: function (x) {
      return 35 + 0.8 * x + 4 * Math.log(x + 1) + 1.5 * Math.sin(0.3 * x);
    },
  ...
  moderado: {
    ...
    derivadaTexto:
      "f'(x) = 45 + 1.6x + 8ln(x + 1) + 5e^(0.05x) + 3sen(0.5x)",
  ...
  critico: {
    ...
    derivadaTexto:
      "f'(x) = 55 + 2.5x + 0.2x² + 12ln(x + 1) + 12e^(0.08x) + 5sen(0.6x)",
...
function calcularCasoActual() {
  ...
  const resultado = metodoNewton(caso, x0, tolerancia, maxIteraciones);
...
function metodoNewton(caso, x0, tolerancia, maxIteraciones) {
  ...
  if (!Number.isFinite(xActual) || xActual <= -1) {
    return crearResultadoError("El valor inicial x0 no es valido. Debe ser mayor que -1.", iteracionesTabla, x0);
  }

  for (let i = 1; i <= maxIteraciones; i++) {
    const fx = caso.f(xActual);
    const dfx = caso.df(xActual);

    if (!Number.isFinite(fx) || !Number.isFinite(dfx)) {
      return crearResultadoError("La funcion no esta definida para el valor actual.", iteracionesTabla, x0);
    }

    if (Math.abs(dfx) < 1e-12) {
      return crearResultadoError("La derivada es muy cercana a cero. El metodo no puede continuar.", iteracionesTabla, x0);
    }
    ...
    if (!Number.isFinite(xNuevo) || xNuevo <= -1) {
      return crearResultadoError("El metodo salto a un valor no valido.", iteracionesTabla, x0);
    }
...
function crearResultadoError(mensaje, iteracionesTabla, x0) {
  return {
    raiz: NaN,
    fx: NaN,
    error: NaN,
...
function mostrarResultado(resultado, caso) {
  ...
  if (!resultado.convergio) {
    analisis.textContent =
      resultado.mensaje +
      " Revisa el valor inicial x0. Si esta muy lejos de la raiz, Newton-Raphson puede fallar.";
    return;
  }
...
function mostrarTabla(iteraciones) {
  ...
  if (iteraciones.length === 0) {
    cuerpo.innerHTML = `<tr><td colspan="6">No hay iteraciones para mostrar.</td></tr>`;
    return;
  }
  ...
  iteraciones.forEach((it) => {
    ...
    fila.innerHTML = `
      <td>${it.iteracion}</td>
      <td>${formatoNumero(it.xActual)}</td>
      <td>${formatoCientifico(it.fx)}</td>
      <td>${formatoCientifico(it.dfx)}</td>
      <td>${formatoNumero(it.xNuevo)}</td>
...
function compararCasos() {
  ...
  nombres.forEach((nombre) => {
    ...
    const resultado = metodoNewton(caso, caso.x0, 0.000001, 100);
...
function actualizarInterpretacionVariables(nombreCaso) {
  ...
  const datos = {
    ideal: {
      ...
      conclusion:
        "En este caso, Newton-Raphson parte de un valor inicial cercano a la raiz y usa la derivada para acercarse rapidamente al dia critico."
    },
    ...
    moderado: {
      ...
      conclusion:
        "En este caso, Newton-Raphson encuentra la raiz antes en el tiempo, lo que muestra que el limite economico se alcanza mas rapido que en el estado ideal."
    },
...
function dibujarGrafica3D(caso, resultado) {
  const canvas = document.getElementById("newtonGraph");
  ...
  if (x0 !== null) {
    dibujarPuntoInicial(ctx, x0, punto3D, origenY);
  }
...
function dibujarPuntoInicial(ctx, x0, punto3D, origenY) {
  const punto = punto3D(x0, 1000, 2);
  ...
  ctx.font = "bold 13px Segoe UI";
  ctx.fillText("x0 = " + x0.toFixed(2), punto.x + 10, punto.y - 10);
}
...
function dibujarLeyenda(ctx, caso) {
  ...
  ctx.font = "bold 14px Segoe UI";
  ctx.fillText("Metodo de Newton-Raphson - Grafica 3D aproximada", 20, 30);
  ...
  ctx.fillStyle = "#374151";
  ctx.fillText("Raiz aproximada", 42, 130);

  ctx.fillStyle = "#111827";
  ctx.font = "bold 12px Segoe UI";
  ctx.fillText(caso.titulo, 20, 154);
}
